...
int main()
{
    ...
    vector<CHE> bunnyHalfEdges;
    vector<unsigned int> bunnyIndices;

    string bunnyPath = "bunny/bun_zipper_res3.ply";
    if (!loadPLY(bunnyPath, bunnyVertices, bunnyHalfEdges, bunnyIndices))
    {
        glfwDestroyWindow(window);
        glfwTerminate();
        return -1;
    ...
    createMeshBuffers(bunnyVertices, bunnyIndices, bunnyVAO, bunnyVBO, bunnyEBO);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        drawObject(depthShader, bunnyVAO, static_cast<GLsizei>(bunnyIndices.size()), sphereModel);
        ...
        drawObject(mainShader, bunnyVAO, static_cast<GLsizei>(bunnyIndices.size()), sphereModel);
