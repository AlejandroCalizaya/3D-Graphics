...
int main() {
    ...
    GLFWwindow *window =
        glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Stanford Bunny - QEM Simplification", nullptr, nullptr);
    if (!window) {
        glfwTerminate();
        return -1;
    }
    ...
    string plyPath = "bunny/bun_zipper_res3.ply";
    cout << "Cargando archivo " << plyPath << "..." << endl;
    if (!loadPLY(plyPath, vertices, halfEdges, faceIndices)) {
        cerr << "Falló la lectura del archivo PLY." << endl;
        return -1;
    }

    int initialFaces = static_cast<int>(faceIndices.size() / 3);
    cout << "Carga completada. Caras iniciales: " << initialFaces << endl;

    int targetFaces = 1000;
    cout << "Simplificando malla a " << targetFaces << " caras..." << endl;
    simplifyMesh(vertices, halfEdges, faceIndices, targetFaces);
    cout << "Simplificacion finalizada. Caras activas: " << faceIndices.size() / 3 << endl;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform3f(glGetUniformLocation(shaderProgram, "color"), 0.85f, 0.65f, 0.45f);
